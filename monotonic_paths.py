...
def find_next_wrd(wrd_idx, ref_idx, word_path, strategy='greedy', steps=30):
	...
	for j,v in zip(cx.col, cx.data):
		...
		if strategy == 'greedy':
			...
			monotonic_vec	= make_monotonic_vec(word_features[ref_idx], word_features[wrd_idx], word_features[j], steps)
			...
			non_mono_count	= 10000 - monotonic(word_features[ref_idx], word_features[wrd_idx], word_features[j], ret='count')
...
def make_monotonic_path(word_path_ids, ref_idx, steps=30, factor=0):
	...
	for i in range(len(final_embs)-1):
		check *= monotonic(final_embs[-1], final_embs[i], final_embs[i+1], ret='bool')
	assert check

	return final_embs

def scale_inputs(input_ids, ref_input_ids, device, auxiliary_data, steps=30, factor=0, strategy='greedy'):
	...
	for idx in range(len(input_ids)):
		...
		monotonic_embs	= make_monotonic_path(word_path, ref_input_ids[idx], steps=steps, factor=factor)
